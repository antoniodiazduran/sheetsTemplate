...
class Urgency extends \Controller {
   ...
   public function edit() {
                $this->f3->set('breadcrumbs','mat/urg');
                $rids = $this->f3->get('PARAMS.id');
                if (strpos($rids,",")>0) {
                        $rids = rtrim($rids,",");
                        $sqlstrm = 'SELECT rid,Epoch,PartNumber,Description,DueDate,Urgency FROM enc_matlog WHERE rid in ('.$rids.')';
                        $record  = $this->db->exec($sqlstrm);
                } else {
                        $record  = $this->db->exec('SELECT rid,Epoch,PartNumber,Description,DueDate,Urgency FROM enc_matlog WHERE rid = ?',$this->f3->get('PARAMS.id'));
                }
                ...
                $this->f3->set('content','materials/urgency.htm');
    }
    public function upd() {
