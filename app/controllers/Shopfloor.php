...
class Shopfloor extends Controller {
    ...
    public function apidbs() {
		...
		$data[] = $this->db->exec('SELECT Line, Description, substr(Customer,1,15) Customer, substr(Customer,1,10), DueDate, Urgency, Requestor, Owner FROM enc_log WHERE display = "y" ORDER BY sort ASC, DueDate ASC');
